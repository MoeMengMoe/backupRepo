...
void scan_and_move(const char* search_dir, const char* dest_dir, int num) {
    ...
    while (fgets(filename, sizeof(filename), fp)) {
        ...
        // Parse input format: LA_num_name_000.pdf
        int file_num = 0;
        int file_id = 0;
        char file_name[MAX_NAME] = {0};
        if (sscanf(basename, "LA_%d_%127[^_]_%d", &file_num, file_name, &file_id) != 3 ||
            file_id < 0 || file_id > 999) {
            printf("Skipped: %s (format must be LA_num_name_000.pdf)\n", filename);
            continue;
        }

        // Search for matching student by parsed name
        int found = 0;
        for (int i = 0; i < student_count; i++) {
            if (_stricmp(file_name, students[i].name) == 0) {
                found = 1;
                // Generate new filename
                char new_filename[MAX_NAME];
                snprintf(new_filename, sizeof(new_filename), 
                         "LA_%d_%s_%03d.pdf", num, students[i].name, students[i].id);
...
int main() {
    ...
    if (student_count == 0) {
        ...
        printf("4. Filename must follow LA_num_name_000.pdf format\n");
        return 1;
    }
    ...
    snprintf(dest_dir, sizeof(dest_dir), "D:\\Assignment\\LA_%d_Class3", num);
